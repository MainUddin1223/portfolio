        ...
        <div className='font-serif  p-8'>
            <div className='grid grid-cols-1 md:grid-cols-2 lg:w-3/4 mx-auto'>
                <div className=' lg:relative'>
                    <div className='bg-orange-600 lg:w-3/4 lg:absolute inset-y-0 right-0'>
                        <div className='text-accent pl-4'>
                ...
                <div className='bg-orange-200 '>
                    <div className='w-3/4 mx-auto'>
                        <form onSubmit={handleSubmit(onSubmit)}>
                            <input {...register("name", { required: true })} placeholder='Your name' className='block w-full my-2 p-2 text-lg rounded' />
                            {errors.name?.type === 'required' && <p className='text-red-500 text-xl'>Please put your name</p>}

                            <input {...register("email", { required: true })} placeholder="Your Email" className='block w-full my-2 p-2 text-lg rounded' />
                            {errors.email && <p className='text-red-500 text-xl my-2'>Please put your email</p>}

                            <textarea {...register("message", { required: true })} placeholder="Message" name="" id="" className='w-full p-2 text-lg rounded-lg block mx-auto' rows="4"></textarea>
                            {errors.message && <p className='text-red-500 text-xl my-2'> Please write something</p>}
                            <input type="submit" value="Contact Me" className='btn btn-accent mx-auto block my-4 text-white' />
                        </form>
                    </div>
                </div>
            </div>
